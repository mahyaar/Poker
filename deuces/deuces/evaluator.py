...
class Evaluator(object):
    ...
    def evaluate(self, cards, board, is_2to7=False):
        """
        ...
        if is_2to7:
            return 7463 - self.hand_size_map[len(all_cards)](all_cards)
    ...
    def get_rank_class(self, hr, is_2to7=False):
        """
    ...
    def hand_summary(self, board, hands, is_2to7=False):
        """
        ...
        for i in range(len(stages)):
            ...
            for player, hand in enumerate(hands):
                ...
                rank = self.evaluate(hand, board[:(i + 3)],is_2to7)

                rank_class = self.get_rank_class(rank, is_2to7)
            ...
            # otherwise on all other streets
                ...
                if len(winners) == 1:
                    ...
                    print "Player %d is the winner with a %s\n" % (winners[0] + 1,
                       self.class_to_string(self.get_rank_class(self.evaluate(hands[winners[0]], board,is_2to7),is_2to7)))
    ...
    def to_river(self, board, deck):
        if len(board) > 4:
            return board, deck
        else:
            next_card = deck.draw()
            board.append(next_card)
            return self.to_river(board, deck)

    def winner_determination(self, board, hands, number_of_players, is_2to7=False):
        """
        Gives a sumamry of the hand with ranks as time proceeds.
        ...
        analysis to make sense.
        """
        #assert len(board) > 2, "Need at least 3 cards on the board"
        assert len(board) < 6, "Too many cards on the board"

        for hand in hands:
            assert len(hand) == 2, "Inavlid cards in hand"

        assert len(hands) <= number_of_players, "too many hands"

        init_board_length = len(board)
        init_hand_length = len(hands)

        winner_count = [0]*number_of_players
        #print(winner_count)
        deck = Deck()
        current_board = []
        for card in board:
        ...
        for i in range(10000):
            Card.print_pretty_cards(current_board)
            # draw existing cards
            ...
            for i in range(len(current_board)):
                if i > init_board_length-1:
                    deck.insert_card(current_board.pop(init_board_length))
            ...
            for hand in hands:
                for card in hand:
                    deck.pop_card(card)
            for i in range(number_of_players-init_hand_length):
                if(len(hands) > init_hand_length):
                    hands.pop(init_hand_length)
            for i in range(number_of_players - init_hand_length):
                unknown_hand = deck.draw(2)
                hands.append(unknown_hand)
            ...
            current_board, deck = self.to_river(current_board, deck)
            #Card.print_pretty_cards(current_board)

            best_rank = 7463  # rank one worse than worst hand
            winners = []
            for player, hand in enumerate(hands):
                rank = self.evaluate(hand, current_board, is_2to7)
                ...
                if rank == best_rank:
                    ...
                    best_rank = rank
            if len(winners)==1:
                winner_count[winners[0]]+=1
                if(winners[0]!=0):
                    print("winner_hand: ")
                    Card.print_pretty_cards(hands[winners[0]])
